...
int main() // 主程式
{
  ...
  poolsz = 256*1024; // 設定記憶體池大小
  if (!(e = malloc(poolsz))) { printf("could not malloc(%d) text area\n", poolsz); return -1; } // 分配程式段記憶體
  if (!(sp = malloc(poolsz))) { printf("could not malloc(%d) stack area\n", poolsz); return -1; }  // 分配堆疊段記憶體

  memset(e, 0, poolsz);
  power = e; // 設定 `power` 函式的起始地址

  // 開始將 `power` 函式的機器碼硬塞到記憶體中
  *e++ = ENT; *e++ = 0; // 函式入口
  *e++ = LLA; *e++ = 2;
  *e++ = LI;
  *e++ = PSH;
  *e++ = IMM; *e++ = 0;
  *e++ = EQ;
  *e++ = BZ; loc = e; *e++ = 0; // if (n == 0) branch to loc
  *e++ = IMM; *e++ = 1;
  *e++ = LEV;
  *e++ = JMP; *e++ = 0; // jump to end
  *loc = (int) e;
